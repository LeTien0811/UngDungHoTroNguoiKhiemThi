...
class IntentClassifierEngine {
  ...
  Future<void> initializer() async {
    try {
      ...
      //Tien note: load model ra chuyen thanh byte de dua vao ram va nap model nhe
  ...
  Future<IntentType> processIntentClassifier(String voiceIntent) async{
    try {
      if(_provider.status != AIStatus.ready){
        developer_log.log("Intent service chua duoc khoi dong tien hanh khoi dong", name: "processIntentClassifier.processIntentClassifier");
        await initializer();
        if(_provider.status != AIStatus.ready){
      ...
      developer_log.log("co loi xay ra", name: "processIntentClassifier.processIntentClassifier");
      _provider.setError();
  ...
  void shutdownEngine() {
    developer_log.log("Tiến hành giải phóng C++ FastText", name: "IntentClassifierEngine");
