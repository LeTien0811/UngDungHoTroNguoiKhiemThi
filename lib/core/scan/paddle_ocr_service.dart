...
class PaddleOcrService {
  ...
  String _extractTextFromResults(List<OcrResult> results) {
    ...
    final sortedResults = List<OcrResult>.from(
      results.where((e) => e.name != null),
    );
    ...
    sortedResults.sort((a, b) {
      ...
      final double aMinY = aBounds
          .map((p) => (p.y ?? 0).toDouble())
          .reduce(min);
      final double bMinY = bBounds
          .map((p) => (p.y ?? 0).toDouble())
          .reduce(min);

      if ((aMinY - bMinY).abs() < 20) {
        final double aMinX = aBounds
            .map((p) => (p.x ?? 0).toDouble())
            .reduce(min);
        final double bMinX = bBounds
            .map((p) => (p.x ?? 0).toDouble())
            .reduce(min);
