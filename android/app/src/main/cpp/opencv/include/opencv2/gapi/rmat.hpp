...
public:
    ...
    // - Doesn't implement copy semantics (it's assumed that a view_models is created each time
    // wrapped data is being accessed);
    // - Has an optional callback which is called when the view_models is destroyed.
    ...
    public:
        ...
        // the view_models when accessed for writing, to ensure that the data from the view_models
        // is transferred to the device when the view_models is destroyed
    ...
    // Note: When accessed for write there is no guarantee that returned view_models
    // will contain actual snapshot of the mapped device memory
    // (no guarantee that fetch from a device is performed). The only
    // guaranty is that when the view_models is destroyed, its data will be
